...
DREAMQAS=/home/sh0/S4068570/DreamQAS/code/WM_QAS
...
CV=/research/data/s4068570/DreamQAS_transfer/2_current/dreamqas_campaign/campaign_v1
LOGS=/research/data/s4068570/DreamQAS_transfer/2_current/dreamqas_campaign/scaleup_v1_logs
...
# 6.3 GB on iona) -- and then a 480 MiB DreamQAS-RL run is refused a card with 7 GB free and the
...
# Measured GPU footprint per arm on iona (nvidia-smi --query-compute-apps, 2026-08-02).
...
# Observed on iona 2026-07-31: a run launched with CUDA_VISIBLE_DEVICES=0 won the
...
for line in "${QUEUE[@]}"; do
  ...
  # then dies with "ModuleNotFoundError: No module named 'numpy'". Observed on iona 2026-07-31;
  ...
  # launched a SECOND time into the same output dir. That happened on iona 2026-07-31 and
