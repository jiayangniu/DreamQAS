...
def fmt(vals):
    ...
    value = f"{x.mean():.3g}±{x.std(ddof=1):.2g}" if len(x) > 1 else f"{x.mean():.3g}"
    return f"{value} (n={len(x)})"
...
def main():
    ...
    print("  frozen best-of-1, mHa, mean ± sample std (ddof=1), actual sample counts. Lower is better.")
    print(f"  Internal arms: common {COMMON_EP:,}-episode budget; model-free control retains its native reward.")
    ...
    for name, pat in INTERNAL:
        ...
        rows[name] = cells
        source = "native-RL" if name == "DreamQAS-RL" else "oracle-free"
        print(f"{name:22s}{source:12s}" + "".join(f"{fmt(c):>22s}" for c in cells))
    print("-" * 122)
    for meth in EXTERNAL:
        ...
        for m in MOLS:
            ...
            counts.append(0 if a is None else a["n"])
        disp = "QuantumDARTS †" if meth == "qdarts" else meth
        cs = ["—" if c is None else f"{c[0]:.3g}±{c[1]:.2g} (n={n})" for c, n in zip(cells, counts)]
    ...
    print("  best-of-1 ≡ best-of-10 by construction; its ± describes variation across supplied independent search runs.")
    ...
    print("  1. Training signals differ: WM arms are oracle-free; model-free control and externals use native rewards;")
    ...
    print("  4. All available seeds remain in the mean; inspect sample counts and missing inputs.")
