...
# --- ORACLE-FREE override: use the changed-reward (empirical-frontier score) training runs ---
import glob as _glob
_OF = "/research/data/s4068570/DreamQAS_transfer/2_current/dreamqas_campaign/oracle_free_v1"
def dq_dirs(method, mol):                               # shadows the imported dq_dirs (seed_curves uses this)
    if method == "Full":    return _glob.glob(f"{_OF}/dreamqas/gru_energy_surrogate_{mol}_s*_of")
    if method == "No-imag": return _glob.glob(f"{_OF}/ablations/gru_energy_none_{mol}_s*_of_noimag")
    if method == "RLQAS":   return _glob.glob(f"{_OF}/dreamqas_rlqas/baseline_analysis/G0_v2_{mol}/seed_*")
    return []

C = "/research/data/s4068570/DreamQAS_transfer/2_current/dreamqas_campaign/campaign_v1"
OUT = f"{HERE}/outputs/main_results"
W, DOWN = 100, 50
# Budget rule (locked 2026-07-27): EVERY task is reported at the COMMON 15,000-episode budget
# (= 3750 iters at 4 real episodes/iter). LiH6q is the only task configured with a 2x budget (7500
# iters / 30k episodes), so it is the only entry here; its full-budget trajectory is an APPENDIX
# extension, not the main-figure basis. This is a rule that can be stated in advance — do NOT retune
# it to a value chosen from the outcome.
CAP_ITERS = {"LiH6q": 3750}
# parallel-descent gap annotation: representative error level (mHa) at which to draw the Full-vs-No-imag
# horizontal VQE arrow (matched-error speedup in the parallel-descent band). BeH2_8q's gap is near-floor.
GAP_Y = {"LiH4q": 1.0, "LiH6q": 15.0, "BeH2_8q": 5.0, "BeH2": 2.0, "BeH2_10q": 5.0}
MAIN3 = ["LiH4q", "LiH6q", "BeH2_8q"]
ALL = ["LiH4q", "LiH6q", "BeH2_8q"]
DISP = {"LiH4q": "LiH (4q)", "LiH6q": "LiH (6q)", "BeH2_8q": "BeH$_2$ (8q)",
        "BeH2": "BeH$_2$ (6q)", "BeH2_10q": "BeH$_2$ (10q)"}
FNAME = {"LiH4q": "LiH_4q", "LiH6q": "LiH_6q", "BeH2_8q": "BeH2_8q",
         "BeH2": "BeH2_6q", "BeH2_10q": "BeH2_10q"}
...
def seed_curves(method, mol):
    ...
    if method in ("Full", "No-imag", "RLQAS"):
        for d in dq_dirs(method, mol):
            ...
            cap = CAP_ITERS.get(mol)                    # cap at `cap` iters (real_eps_per_iter=4 eps/iter)
            if cap and eps:
                eps = eps[: cap * 4]
...
def draw_gap_arrow(ax, mol):
    """Horizontal Full-vs-No-imag VQE arrow at a representative error level in the parallel-descent band:
    matched-error speedup = cum_VQE(No-imag) / cum_VQE(Full) to first reach GAP_Y[mol]."""
    y = GAP_Y.get(mol)
    if y is None:
        return
    aF, aN = aggregate("Full", mol), aggregate("No-imag", mol)
    if aF is None or aN is None:
        return
    xF, xN = cross((aF[0], aF[1]), y), cross((aN[0], aN[1]), y)
    if not (np.isfinite(xF) and np.isfinite(xN) and xN > xF):
        return
    col = "#444"
    ax.annotate("", xy=(xF, y), xytext=(xN, y), zorder=7,
                arrowprops=dict(arrowstyle="<|-|>", color=col, lw=1.4, shrinkA=0, shrinkB=0))
    for cx in (xF, xN):
        ax.plot([cx], [y], marker="|", color=col, ms=9, mew=1.6, zorder=7)
    ax.annotate(f"{xN / xF:.1f}× fewer VQE", xy=((xF * xN) ** 0.5, y), xytext=(0, 7),
...
def main():
    ...
    sp = {}                       # oracle-free: NO Q_b speedup box (empty -> draw_panel skips it); use draw_gap_arrow instead
    # ---- figure ----
    fig, axes = plt.subplots(1, 3, figsize=(13.2, 4.4))
    handles = {}
    for ax, mol in zip(axes, MAIN3):
        handles.update(draw_panel(ax, mol, sp))
        draw_gap_arrow(ax, mol)       # parallel-descent matched-error VQE gap arrow
    ...
    for ext in ("png", "pdf"):
        fig.savefig(f"{OUT}/speed_main_3mol_w100_oraclefree.{ext}", dpi=(160 if ext == "png" else None), bbox_inches="tight")
    plt.close(fig)
    print(f"[written] speed_main_3mol_w100_oraclefree.{{png,pdf}} in {OUT} (oracle-free data, gap arrows)")

    # ---- second figure: BeH2-6q + BeH2-10q (same oracle-free pipeline + gap arrows) ----
    fig2, axes2 = plt.subplots(1, 2, figsize=(9.0, 4.4))
    h2 = {}
    for ax, mol in zip(axes2, ["BeH2", "BeH2_10q"]):
        h2.update(draw_panel(ax, mol, {}))
        draw_gap_arrow(ax, mol)
    axes2[0].set_ylabel("training episode-best error (mHa, 100-ep moving mean)", fontsize=10)
    hs2 = [h2[m] for m in METHODS if m in h2]
    fig2.legend(hs2, [h.get_label() for h in hs2], loc="upper center", ncol=5, fontsize=10.5,
                frameon=False, bbox_to_anchor=(0.5, 1.03), handlelength=2.6, columnspacing=1.6)
    fig2.tight_layout(rect=(0, 0.02, 1, 0.92))
    for ext in ("png", "pdf"):
        fig2.savefig(f"{OUT}/speed_beh2_6q_10q_w100_oraclefree.{ext}", dpi=(160 if ext == "png" else None), bbox_inches="tight")
    plt.close(fig2)
    print(f"[written] speed_beh2_6q_10q_w100_oraclefree.{{png,pdf}} in {OUT}")
    return sp


def _unused_speedup_txt(sp):     # canonical rq2_speedup_w100.txt is NOT regenerated for the oracle-free figure
    L = ["=" * 112,
